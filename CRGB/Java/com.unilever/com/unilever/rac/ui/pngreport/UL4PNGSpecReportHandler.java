/* Source File Name:   UL4PAMSpecReportHandler
 *
...
/**
 * The Class UL4PAMSpecReportHandler.
 */


public class UL4PNGSpecReportHandler extends AbstractHandler  implements IHandler
{
    ...
    private InterfaceAIFComponent selection0            = null ;
    ...
    {       
    	TCComponentItemRevision  pmlrevision    = null ;
    	...
    	try 
    	{
            ...
            selection0 = AIFUtility.getActiveDesktop().getCurrentApplication().getTargetComponent();
		        
		   // InterfaceAIFComponent[] arrayOfInterfaceAIFComponent = SelectionHelper.getTargetComponents(localISelection);
			selection = (TCComponent) selection0;
			
			String parentType = selection.getTypeComponent().getParent().toString();
				
			if(parentType.equals(UL4Common.PMLREVISION))
			{
				pmlrevision = (TCComponentItemRevision) selection;
			}
			
			boolean imgRequired = false ;
			
			if(imageDatasetAvialable(pmlrevision))
			{
			...
			UL4PNGSpecReportOperation pdf = new UL4PNGSpecReportOperation(pmlrevision,session,imgRequired);
  		...
  		try
  		{
  			...
  			for( int inx=0 ; inx < component.length ; inx++)
  			{
  				if( component[inx].getComponent()  instanceof TCComponentDataset)
  				{
  					...
  					if(type.equals("U4_StructuredPDF") || type.equals("U4_PrintPDF"))
  						return true ;
  				}				
  			}
  		}
