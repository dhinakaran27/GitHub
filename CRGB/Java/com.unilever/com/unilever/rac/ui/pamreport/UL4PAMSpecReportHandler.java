...
public class UL4PAMSpecReportHandler extends AbstractHandler  implements IHandler
{
    	...
    	try 
    	{
			...
			if(parentType.equals(UL4Common.PAMREVISION))
			{
				...
				for( int inx=0 ; inx < aifcomponent.length ; inx++)
				{
					...
					if( component instanceof TCComponentItemRevision && ((TCComponentItemRevision)component).getTypeComponent().getParent().getType().equals(UL4Common.PACKREVISION) )
					{
			...
			else if(parentType.equals(UL4Common.PACKREVISION))
			{
			...
			{	
				...
				UL4PAMSpecReportOperation pdf = new UL4PAMSpecReportOperation(packrevision,pamrevision,session,imgRequired);
